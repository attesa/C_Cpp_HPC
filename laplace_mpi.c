/* This program is to solve the Laplace function with MPI */
#include "stdio.h"
#include "mpi.h"
#include "unistd.h"
#include "math.h"
...
int main(int argc, char **argv)
{
  ...
  fp = fopen("mpi_laplace_result_10x40.txt","w");
  MPI_Status status;
  MPI_Request request;
  MPI_Init(&argc,&argv);
  ...
  while (condition[0]) {
    ...
    {
      for (i=rank*size_factor;i<N-1;i++) {
        for (j=1;j<N-1;j++) {
          phi_new[i][j] = (phi_old[i-1][j]+phi_old[i+1][j]+phi_old[i][j+1]+phi_old[i][j-1])/4.0;
          local_diff[0] = local_diff[0] + pow((phi_new[i][j]-phi_old[i][j]),2.0);
  	}
    ...
    if (rank==0)
    {
      ierr = MPI_Isend(&phi_old[size_factor-1],N,MPI_DOUBLE,rank+1,1,MPI_COMM_WORLD,&request);
      ierr = MPI_Irecv(&phi_old[size_factor],N,MPI_DOUBLE,rank+1,1,MPI_COMM_WORLD,&request);
...
//      printf("%f  %d \n",diff[0],iter);
  ...
  for (isize=size_factor;isize<N;isize++) {
      trank=isize/size_factor; 
      if (rank==trank) {
	ierr = MPI_Send(&phi_old[isize],N,MPI_DOUBLE,0,trank,MPI_COMM_WORLD);       
      }
      if (rank==0) {
        ierr = MPI_Recv(&phi_old[isize],N,MPI_DOUBLE,trank,trank,MPI_COMM_WORLD,&status);
      }
   }
  MPI_Barrier(MPI_COMM_WORLD);
